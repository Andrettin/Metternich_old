...
void trade_route::add_path_province_points_to_qvariantlist(QVariantList &point_list, const province *path_province, const QPoint &start_map_pos, const province *next_path_province)
{
	...
	if (next_path_province != nullptr) {
		...
		try {
			const pathfinder *pathfinder = path_province->get_world()->get_pathfinder();
			const find_province_pos_path_result result = pathfinder->find_province_pos_path(path_province, next_path_province);

			if (result.success) {
				for (const QPoint &path_pos : result.path) {
					if (path_pos == main_pos || path_pos == next_main_pos) {
						continue;
					}

					point_list.append(path_pos - start_map_pos);
				}
			}
		...
		point_list.append(next_main_pos - start_map_pos);
	}
}
